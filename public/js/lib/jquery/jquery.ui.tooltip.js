  ...
  jQuery.fn.bt = function(content, options) {
    ...
    return this.each(function(index) {
      ...
      this.btOn = function () {
        ...
        var $text = $('<div class="bt-content"></div>').append(content).css({padding: opts.padding, position: 'absolute',  zIndex: opts.textzIndex, left: shadowShiftX, top: shadowShiftY}).css(opts.cssStyles);
        // create the wrapping box which contains text and canvas
        // put the content in it, style it, and append it to the same offset parent as the target
        var $box = $('<div class="bt-wrapper"></div>').append($text).addClass(opts.cssClass).css({position: 'absolute', width: '240px',  zIndex: opts.wrapperzIndex, visibility:'hidden'}).appendTo(offsetParent);
...
//    width:            '150px',               // width of tooltip box
    
    padding:          '10px',                // padding for content (get more fine grained with cssStyles)
    spikeGirth:       8,                    // width of spike
    spikeLength:      9,                    // length of spike
    ...
    wrapperzIndex:    1001,
    offsetParent:     'body',                  // DOM node to append the tooltip into.
    ...
    fill:             "#FFF9CB",  // fill color for the tooltip box, you can use any CSS-style color definition method
                                             // http://www.w3.org/TR/css3-color/#numerical - not all methods have been tested
    
    windowMargin:     40,                    // space (px) to leave between text box and browser edge
  
    strokeWidth:      1,                     // width of stroke around box, **set to 0 for no stroke**
    strokeStyle:      "#4A4A4A",                // color/alpha of stroke
  
    cornerRadius:     3,                     // radius of corners (px), set to 0 for square corners
    
                      // following values are on a scale of 0 to 1 with .5 being centered
    
    centerPointX:     .4,                    // the spike extends from center of the target edge to this point
    centerPointY:     .10,                   // defined by percentage horizontal (x) and vertical (y)
      
    shadow:           false,                 // use drop shadow? (only displays in Safari and FF 3.1) - experimental
    shadowOffsetX:    1,                     // shadow offset x (px)
    shadowOffsetY:    1,                     // shadow offset y (px)
    shadowBlur:       1,                     // shadow blur (px)
    ...
    contentSelector:  "$(this).prevAll(':first').text()", // if there is no content argument, use this selector to retrieve the title
