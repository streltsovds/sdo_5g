...
elFinder.prototype.view = function(fm, el) {
	...
	*/
	this.renderIcon = function(f) {
		var str = '<p'+(f.tmb ? ' style="'+"background:url('"+f.tmb+"') 0 0 no-repeat"+'; background-size: cover;"' : '')+'/><label>'+this.formatName(f.name)+'</label>';
	...
	*/
	this.mime2class = function(mime) {
        mime = mime.replace('/' , ' ').replace(/\./g, '-');
        if (mime) mime = 'el-'+mime;
		return mime;
	}
