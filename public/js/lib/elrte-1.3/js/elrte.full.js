...
function elDialogForm(o) {
	...
	var defaults = {
		...
		dialog    : {
			...
			dialogClass: 'el-dialogform-container', //equalteam 
			// XXX: END added code
			autoOpen  : false,
			modal     : true,
			resizable : false,
			closeOnEscape : true,
			// XXX: Changed buttons object representation from object to array
			//      Changed label of cancel button to translated value
			buttons   : [{
				text: o.rte.i18n('Cancel'),
	...
	if ($.browser.msie) {
		this.$doc.bind('keyup', function(e) {
			if (e.keyCode == 86 && (e.metaKey||e.ctrlKey)) {
				...
				this.ui.update();
				// XXX: I don't know why, but in our code this changed to self
				// self.ui.update();
			}
		});
	}
			...
			html = html
				...
				.replace(this.serviceClassRegExp, function(t, n, a, e) {
					...
					if (a['class']['elrte-google-maps']) {
						...
						// XXX: BEGIN code added by someone
						var needInject = true;
						var src = '';
						...
						// XXX: END code added by someone
						j.params && $.each(j.params, function(i, p) {
							// XXX: BEGIN added code
							if (p.name == "FlashVars") {
								needInject = false;
								p.value = 'src='+src+'&playButtonOverlay=false&controlBarAutoHide=false';
							}
							// XXX: END added code
							o = '<param '+self.serializeAttrs(p)+">\n";
							// XXX: BEGIN added code
							if (j.embed) {
								j.embed[p.name] = p.value;
							}
							// XXX: END added code
						});
						// XXX: BEGIN added code
						var playerUrl = '/js/StrobeMediaPlayback.swf';
						if (needInject) {
							o += '<param name="movie" value="'+playerUrl+'"/>'+"\n";
							o += '<param name="allowFullScreen" value="true"/>'+"\n";
							o += '<param name="FlashVars" value="src='+src+'&playButtonOverlay=false&controlBarAutoHide=false" />'+"\n";
							j.embed.FlashVars = 'src='+src+'&playButtonOverlay=false&controlBarAutoHide=false';
						}
						if (j.obj.width == undefined) {
							j.embed.width = j.obj.width = 360;
						}
						if (j.obj.height == undefined) {
							j.embed.height = j.obj.height = 240;
						}
						if (j.embed) {
							j.embed.src = playerUrl;
						}
						// XXX: END added code
...
(function($) {
	...
	elRTE.prototype.ui.prototype.buttons.flash = function(rte, name) {
		...
		this.src = {
			...
			// XXX: BEGIN added code
			type   : $('<input type="hidden" name="type" value="application/x-shockwave-flash"/>'),
			// XXX: END added code
			// XXX: BEGIN commented code
...
elRTE.prototype.ui.prototype.buttons.link = function(rte, name) {
	...
	this.updateOnclick = function () {
		...
		// XXX: this will be always false, because we commented `use` attribute
		...
		// XXX: END commented code
		// XXX: BEGIN added code
		} else if (this.src.events.onclick) {
		// XXX: END added code
