...
$.fn.abstractFullCalendar = function(options) {
...
function Calendar(element, options, eventSources) {
    ...
    // преобразуем порядковый день событий в число
    for (var i = 0; i < eventSources.length; i++) {
        for (var j = 0; j < eventSources[i].length; j++) {
            var eventConfig = eventSources[i][j];
            eventConfig.start = new Date(1970, 0, eventConfig.start);
            
            if (!eventConfig.end) {
                eventConfig.end = eventConfig.start;
            } else {
                eventConfig.end = new Date(1970, 0, eventConfig.end);
            }
...
function MonthView(element, calendar) {
	...
	function render(date, delta) {
		if (delta) {
            addDays(date, delta * 28);
			//date.setDate(1);
		}
		var start = cloneDate(date, true);
		//start.setDate(1);
		var end = addDays(cloneDate(start), 28);
...
function BasicView(element, calendar, viewName) {
	...
	function buildTable(showNumbers) {
        ...
        if (showWeekNumbers) {
            html += "<th class='fc-week-number " + headerClass + "' style='max-height: 1px;'/>";
        }

        for (i=0; i<colCnt; i++) {
            html += "<th class='fc-day-header fc-" + dayIDs[i] + " " + headerClass + "' style='max-height: 1px;'/>";
        }
		...
		for (i=0; i<rowCnt; i++) {
			...
			for (j=0; j<colCnt; j++) {
				...
				html += "<td" +
				        ...
				        "<div>";
				if (showNumbers) {
					html += "<div class='fc-day-number'>" + (Math.round(cellDate/(60000*60*24)) + 1) + "</div>";
				}
	...
	*/
	...
	function eventDrop(e, event, dayDelta, minuteDelta, allDay, ev, ui) {
        ...
        var startDay = new Date(1970, 0, 1),
            dayNumberStart = (((event.start - startDay)/(1000 * 60 * 60 * 24)) ^ 0) + dayDelta + 1,
            dayNumberEnd   = ((((event.end || event.start) - startDay)/(1000 * 60 * 60 * 24)) ^ 0) + dayDelta + 1,
            dayCount = dayNumberEnd - dayNumberStart + 1;
        
		moveEvents(eventsByID[eventId], dayDelta, minuteDelta, allDay);
		// const f = options["eventDrop"];
		// console.log(event);
		// console.log(dayDelta);
		// console.log(minuteDelta)
		// console.log(allDay)
		// f(event, dayDelta, minuteDelta, allDay);
	...
	function eventResize(e, event, dayDelta, minuteDelta, ev, ui) {
        ...
        // порядковый день:
        var startDay = new Date(1970, 0, 1),
            dayNumberStart = (((event.start - startDay)/(1000 * 60 * 60 * 24)) ^ 0) + 1,
            dayNumberEnd   = ((((event.end || event.start) - startDay)/(1000 * 60 * 60 * 24)) ^ 0) + 1,
            dayCount = dayNumberEnd - dayNumberStart + 1;
