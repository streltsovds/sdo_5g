 ...
 *
 * @property HM_Websocket_Library_Server $server
 * @property HM_Websocket_Library_Server $socket
 */
class HM_Websocket_Library_Connection
{
    private $server;
    private $socket;
	...
	private function hybi10Decode($data)
	{
		...
		else
		{
			...
			$decodedData['payload'] = mb_substr($data, $payloadOffset, null, '8bit');
		}
		return $decodedData;
	}
    ...
    public function _cookie_parse( $line ) {
        $cookies = array();
        $csplit = explode( ';', $line );
        $cdata = array();
        ...
        foreach($cookies as $cookie) {
            $this->cookies[$cookie['value']['key']] = $cookie['value']['value'];
        }
    }
