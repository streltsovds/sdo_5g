...
class HM_Oauth_Provider
{
     ...
     * @var array
     */
    static protected $errnames = array(
     ...
     * @return string
     */
	static public function reportProblem(Zend_Oauth_Exception $e)
	{
	    $code = $e->getCode();
	    if($code == self::PARAMETER_ABSENT) {
	        return "oauth_problem=parameter_absent&oauth_parameters_absent=".self::$problem;
	    }
	    if($code == self::INVALID_SIGNATURE) {
	        return "oauth_problem=signature_invalid&debug_sbs=".self::$problem;
	    }
	    if(isset(self::$errnames[$code])) {
            return "oauth_problem=".self::$errnames[$code];
        }
     ...
     */
	static public function generateToken($size)
	{
	    $str = '';
