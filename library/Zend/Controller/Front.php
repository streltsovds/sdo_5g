...
class Zend_Controller_Front
{
    ...
    public function dispatch(Zend_Controller_Request_Abstract $request = null, Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if (null !== $response) {
            ...
            $xFrameOptionsValue = Zend_Registry::get('config')->http->header->x_frame_options;
            if ($xFrameOptionsValue) $response->setHeader('X-Frame-Options', $xFrameOptionsValue);

            $xXssProtectionValue = Zend_Registry::get('config')->http->header->x_xss_protection;
            if ($xXssProtectionValue) $response->setHeader('X-XSS-Protection', $xXssProtectionValue);

            $xContentTypeOptionsValue = Zend_Registry::get('config')->http->header->x_content_type_options;
            if ($xContentTypeOptionsValue) $response->setHeader('X-Content-Type-Options', $xContentTypeOptionsValue);

            $accessControlAllowMethodsValue = Zend_Registry::get('config')->http->header->access_control_allow_methods;
            if ($accessControlAllowMethodsValue) $response->setHeader('Access-Control-Allow-Methods', $accessControlAllowMethodsValue);

            $accessControlAllowOriginValue = Zend_Registry::get('config')->http->header->access_control_allow_origin;
            if ($accessControlAllowOriginValue) $response->setHeader('Access-Control-Allow-Origin', $accessControlAllowOriginValue);

            $contentSecurityPolicyValue = $this->applyOptions(Zend_Registry::get('config')->http->header->content_security_policy);
            if ($contentSecurityPolicyValue) $response->setHeader('Content-Security-Policy', $contentSecurityPolicyValue);
    ...
    public function applyOptions($string)
    {
        $optionsFrameSrc = Zend_Registry::get('serviceContainer')->getService('Option')->getOption('ext_pages_videos_allowed_domains');
        $optionsImgSrc   = Zend_Registry::get('serviceContainer')->getService('Option')->getOption('images_allowed_domains');

        $eclassServer = Zend_Registry::get('config')->eclass->webinarHost;
        $proctoringServer = Zend_Registry::get('config')->proctoring->host;

        // Михаил указал брать значение параметра полностью
//        if (false !== strpos(Zend_Registry::get('config')->eclass->webinarHost, 'http')) {
//            list(,$eclassServer) = explode('://', Zend_Registry::get('config')->eclass->webinarHost);
//        }

        $webSocketHost = Zend_Registry::get('config')->websocket->host;
        $webSocketPort = Zend_Registry::get('config')->websocket->port;
        $webSocketProtocol = (boolean) Zend_Registry::get('config')->websocket->ssl ? 'wss://' : 'ws://';
        ...
        foreach ($parts as $part) {
            ...
            $attribute = array_shift($subParts);
            if ($attribute == 'frame-src') {
                $result[] = $attribute . ' ' . '\'self\'' . ' ' . $eclassServer . ' ' . $proctoringServer . ' ' . $optionsFrameSrc;
            } elseif ($attribute == 'img-src') {
                $result[] = $attribute . ' ' . '\'self\'' . ' blob: data: ' . $optionsImgSrc;
            } elseif ($attribute == 'connect-src') {
                $result[] = $attribute . ' ' . '\'self\' blob: data: ' . ' ' . $webSocketProtocol . $webSocketHost . ':' . $webSocketPort;
            } else {
                $result[] = $attribute . ' ' . implode(' ', $subParts);
            }
        }
