...
class Zend_Application
{
            ...
            if (defined('APPLICATION_MODULE')) {
                if (APPLICATION_MODULE == 'MOBILE') {
                    ...
                    foreach($unsets as $unsetKey) {
                        unset($options['resources']['frontController']['plugins'][$unsetKey]);
                    }
    ...
    public function run()
    {
        if (version_compare(PHP_VERSION, '7.3.0') >= 0) {
            $cookiesConfig = Zend_Registry::get('config')->cookies;
    //        foreach ($cookiesConfig as $cookie => $value) {
    //            setcookie($cookie, $value);
    //        }

            $config = new SessionConfig();
            $options = [
                'cookieHttpOnly' => $cookiesConfig->httponly,
                'cookieSecure'   => $cookiesConfig->secure,
            ];
            $options['cookieSameSite'] = $cookiesConfig->samesite;
