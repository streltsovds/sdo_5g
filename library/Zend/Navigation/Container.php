...
abstract class Zend_Navigation_Container implements RecursiveIterator, Countable
{
    ...
    public function addPages($pages)
    {
        ...
        $event  = new sfEvent(null, HM_Extension_ExtensionService::EVENT_FILTER_MENU);

        /** @var sfEventDispatcher $service */
        $service = Zend_Registry::get('serviceContainer')->getService('EventDispatcher');

        foreach ($pages as $page) {
            $service->filter($event,  $page);
            $page = $event->getReturnValue();
            if (isset($page['actions'])) {
                $actionEvent = new sfEvent(null, HM_Extension_ExtensionService::EVENT_FILTER_CONTEXT_MENU);
                $service->filter($actionEvent,  $page);
                $page = $actionEvent->getReturnValue();
            }
            $this->addPage($page);
        }
