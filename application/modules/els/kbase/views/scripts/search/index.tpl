    ...
    <v-card-text>
        <?php if($this->error == false): ?>
            <?php
                ...
                $i =0;
            ?>
            <ol class="search-results" start="<?php echo $page * $itemPerPage + 1; // @todo: кажется оно deprecated?>">
            <?php
                foreach($this->resultItems as $key => $item){
                    // здесь было много лишнего кода
                    if($item) //#18208 - ссылка в индексе на документ есть, а его уже в базе нет
                    echo $this->searchItem($item, $page * $itemPerPage + (++$i), $this->words, array('search_query', 'page'));
               }
            ?>
            </ol>
