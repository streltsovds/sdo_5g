...
class Assign_AtmanagerController extends HM_Controller_Action_Assign
{
    ...
    public function indexAction()
    {
        ...
        if ($switcher) {
            $select->joinLeft(array('a' => 'at_managers'), 'a.user_id = t1.MID', array('assigned' => 'a.user_id', 'enabled' => 'a.user_id'));
        } else {
            $select->join(array('a' => 'at_managers'), 'a.user_id = t1.MID', array());
        }

        $select->joinLeft(
            array('r' => 'responsibilities'),
            'a.user_id = r.user_id AND r.item_type = ' . HM_Responsibility_ResponsibilityModel::TYPE_STRUCTURE,
            array(
                'role' => 'a.user_id',
                'departments' => new Zend_Db_Expr('GROUP_CONCAT(item_id)'),
            )
        )
            ->group(array('t1.MID', 't1.LastName', 't1.FirstName', 't1.Patronymic', 't1.blocked', 't1.Login', 'a.user_id'));
            ...
            array(
                ...
                'role' => array(
                    ...
                    'callback' => array(
                        ...
                        'params' => array('{{departments}}', '{{enabled}}')
                    )
                ),
                'departments' => array(
                ...
                'fio' => array(
                    'title' => _('ФИО'),
                    'decorator' => $this->view->cardLink($this->view->url(array('module' => 'user', 'controller' => 'list', 'action' => 'view', 'gridmod' => null, 'user_id' => '')) . '{{MID}}', _('Карточка пользователя')) . '<a href="' . $this->view->url(array('module' => 'user', 'controller' => 'report', 'action' => 'index', 'gridmod' => null, 'user_id' => '')) . '{{MID}}' . '">' . '{{fio}}</a>',
                    'callback' =>
        ...
        $grid->setGridSwitcher([
            ...
            'title' => _('Показать всех менеджеров по оценке'),
        ...
        if (strlen($postMassIds)) {
            ...
            if (count($ids)) {
                ...
                foreach ($ids as $id) {
                    ...
                    $fetch = $this->getService('AtManager')->fetchAll(array('user_id = ?' => $id));
                    try {
                        if (count($fetch) == 0) {
        ...
        if (strlen($postMassIds)) {
            ...
            if (count($ids)) {
                foreach ($ids as $id) {
                    ...
                    $this->getService('AtManager')->deleteBy(
