...
class HM_Form_LessonStep1 extends HM_Form_SubForm
{
    public function init()
    {
        ...
        */
        ...
        $forumEventId = HM_Activity_ActivityModel::ACTIVITY_FORUM;
        $webinarEventId = HM_Event_EventModel::TYPE_WEBINAR;
        $taskEventId = HM_Event_EventModel::TYPE_TASK;

        $loadTestSettingsScript = <<<SCRIPT
            $(document).ready(function() {
                if($("#step1").length<1) return;
                
                if (\$('#event_id').val() == $forumEventId || 
                    \$('#event_id').val() == $webinarEventId /*|| 
                    \$('#event_id').val() == $taskEventId*/
                ) {
                    $('#has_proctoring').prop('disabled', true);                
                }
                   
                $("#step1").delegate('#event_id', 'change', function() {
                    var \$this = $(this);
                    
                    if (\$this.val() == $forumEventId || 
                        \$this.val() == $webinarEventId /*||
                        \$this.val() == $taskEventId*/
                    ) {
                        $('#has_proctoring').prop('disabled', true);
                    }
                    else {
                        $('#has_proctoring').prop('disabled', false);
                    }
                });
            })
SCRIPT;
