<?php
class Quest_MeetingController extends HM_Controller_Action_Multipage_Quest
{
    const NAMESPACE_MULTIPAGE = 'meeting-multipage';
    ...
    public function _redirectToMultipage($msg = '')
    {
        ...
        if (!$this->isAjaxRequest()) {
            $this->_redirector->gotoSimple('view', 'meeting', 'quest', array('quest_id' => $this->_getMultipageId()));
        }
    }

    public function _getBaseUrl()
    {
        return array('module' => 'quest', 'controller' => 'meeting');
    }
    ...
    public function _getPersistentModel($mode = null, $contextEventId = null, $contextEventType = null)
    {
        ...
        $model = parent::_getPersistentModel($mode, $meetingId, HM_Quest_Attempt_AttemptModel::CONTEXT_TYPE_PROJECT);// Возвращает модель тек. попытки тестирования

        if ($meetingId) {
            $meeting = $this->getService('Meeting')->getOne($this->getService('Meeting')->fetchAll(array('meeting_id = ?' => $meetingId)));
            $model->setContextModel($meeting);
        }
