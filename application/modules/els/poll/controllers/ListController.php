...
class Poll_ListController extends HM_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        if ($subjectId) {
            ...
            $select->from(
                    array('t' => 'quizzes'),
                    array('t.quiz_id', 't.title', 't.location', 'subject' => 's.subject_id', 't.questions', 't.updated', 'tags' => 't.quiz_id'));
        ...
        if($subjectId > 0){
            ...
            $grid->addMassAction(
                array('module' => 'poll', 'controller' => 'list', 'action' => 'setrating'),
                _('Использовать оценку в построении рейтинга')
            );
            $grid->addMassAction(
                array('module' => 'poll', 'controller' => 'list', 'action' => 'unsetrating'),
                _('Убрать оценку из построении рейтинга')
            );
        }
...
//        }
    ...
    public function create(Zend_Form $form)
    {
        ...
        if ($tags = $form->getParam('tags')) {
            $this->getService('Tag')->updateTag( $tags, $quiz->quiz_id, $this->getService('TagRef')->getPollType() );
        }
...
/*        if(!$quiz->isEditAllowed($subjectid, $userRole)){
    ...
    {
        $this->_setRatingStatus(true);
    }

    public function unsetratingAction()
    {
        $this->_setRatingStatus(false);
    }

    private function _setRatingStatus($calcRationg = false)
    {
        $this->_helper->getHelper('layout')->disableLayout();
        Zend_Controller_Front::getInstance()->unregisterPlugin('HM_Controller_Plugin_Unmanaged');
        ...
        if (strlen($postMassIds)) {
            ...
            if (count($ids)) {
                $data = array(
                    'calc_rating' => intval($calcRationg)
                );
                $updateWhere = $this->quoteInto(
                    ...
                    array(
                        $ids,
                        HM_Poll_PollModel::STATUS_CONTENTONLY
                    )
                );
                $this->getService('Poll')->updateWhere($data, $updateWhere);
            }
        }
