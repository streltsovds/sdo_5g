<?php
class HM_Form_MeetingStep2 extends HM_Form_SubForm
{
    private $_event;
    private $_step1;
    ...
    public function init()
    {
                ...
                if (isset($activities[$session['step1']['event_id']])) {
                    ...
                    if (strlen($activityService)) {
                        ...
                        if ($service instanceof HM_Service_Schedulable_Interface) {
                            $service->onCreateMeetingForm($this, 'project', $session['step1']['project_id'],$session['step1']['title']);
                        }
                    }
                }
                break;
        ...
        if (
            !$this->getDisplayGroup('MeetingGroup') &&
        ...
        if (in_array($session['step1']['event_id'], array(HM_Event_EventModel::TYPE_COURSE, HM_Event_EventModel::TYPE_RESOURCE))) {
            $this->addElement(
                ...
                array(
                    ...
                    'Required' => false,
                    'Label' => '',
                    'MultiOptions' => HM_Activity_ActivityModel::getMeetingActivities()
                )
            );

            $this->addDisplayGroup(
                array('activities'),
                'MeetingGroupActivities',
    ...
    public function initCourse()
    {
        ...
        $collection = $this->getService('Project')->getCourses($this->getParam('project_id', 0));
        if (count($collection)) {
            $courses = $collection->getList('project_id', 'Title', _('Выберите учебный модуль'));
        }
        ...
        if ($this->_step1['vedomost']) {
            $scaleId = isset($this->_event) ?  $this->_event->scale_id : HM_Meeting_Course_CourseModel::getDefaultScale();
            $description = '';
            switch ($scaleId) {
    ...
    public function initLecture()
    {
        $this->addElement($this->getDefaultTreeSelectElementName(), 'module', array(
            ...
            'filters' => array('int'),
            'params' => array(
                'remoteUrl' => $this->getView()->url(array('module' => 'meeting', 'controller' => 'ajax', 'action' => 'modules-list'))
            )
        ));
...
SCRIPT;
        ...
        $projectId = (int) $this->getParam('project_id', 0);
        ...
        $this->addElement($this->getDefaultSelectElementName(), 'module', array(
            ...
            'filters' => array('int'),
            'multiOptions' => $testsExt,
            'OptionType' => HM_Event_EventModel::TYPE_TEST
        ));

        if ($this->_step1['vedomost']) {

            $scaleId = isset($this->_event) ?  $this->_event->scale_id : HM_Meeting_Test_TestModel::getDefaultScale();
            $description = '';
            switch ($scaleId) {
                case HM_Scale_ScaleModel::TYPE_BINARY:
                ...
                case HM_Scale_ScaleModel::TYPE_CONTINUOUS: // @todo: добавить TYPE_DISCRETE; расчет по формуле характерен для TYPE_DISCRETE
                    ...
                    $collection = $this->getService('Formula')->fetchAll(
                        $this->getService('Formula')->quoteInto(
                            ...
                            array(HM_Formula_FormulaModel::TYPE_MARK, $projectId)
                        ),
                        'name'
                    );
        ...
        $collection = $this->getService('Formula')->fetchAll(
            $this->getService('Formula')->quoteInto(
                array('type = ?', ' AND (cid = ? OR cid = 0)'),
                array(HM_Formula_FormulaModel::TYPE_GROUP, $projectId)
            ),
            'name'
        );
        ...
        $meetingId  = (int) $this->getParam('meeting_id',  0);
        ...
        if ($questId === null) {
            ...
            if (!empty($session['step2'])) {
                ...
                $meeting = $this->getService('Meeting')->find($meetingId)->current();
        ...
        if ($questId && (isset($testsOwn[$questId]) || isset($testsExt[$questId]))) {
            ...
            if (!empty($questId)) {
                ...
                $meetingScope = HM_Quest_QuestModel::SETTINGS_SCOPE_MEETING;

                if ($meetingId && $quest->hasScopeSettings($meetingScope, $meetingId)) {
                    $quest->setScope($meetingScope, $meetingId);
                } else {
...
SCRIPT;
    ...
    public function initWebinar()
    {
        ...
        $webinars = $this->getService('Webinar')->fetchAll($this->getService('Webinar')->quoteInto('subject_id = ? AND subject = \'project\'' , $projectId,'project'));
    ...
    public function initResource()
    {
        ...
        $collection = $this->getService('Resource')->fetchAllDependenceJoinInner(
            'SubjectAssign',
            $this->getService('Resource')->quoteInto('SubjectAssign.subject_id = ? AND SubjectAssign.subject=\'project\'', $projectId)
        );
        ...
        if ($this->_step1['vedomost']) {
            $scaleId = isset($this->_event) ?  $this->_event->scale_id : HM_Meeting_Resource_ResourceModel::getDefaultScale();
            $description = '';
            $disabled = false;
            switch ($scaleId) {
                case HM_Scale_ScaleModel::TYPE_BINARY:
     ...
     public function initTask()
    {
        ...
        $collection = $this->getService('Task')->fetchAllDependenceJoinInner(
            'ProjectAssign',
            $this->getService('Task')->quoteInto(array('ProjectAssign.project_id = ?', ' AND questions > ?'), array($projectId, 0))
        );
        ...
        $collection = $this->getService('Course')->fetchAllDependenceJoinInner(
            'ProjectAssign',
            $this->getService('Course')->quoteInto('project_id = ?', $projectId)
        );

        $courses = $collection->getList('project_id', 'Title');
        ...
        $this->addElement($this->getDefaultSelectElementName(), 'assign_type', array(
            ...
            'validators' => array('Int'),
            'filters' => array('int'),
            'multiOptions' => HM_Meeting_Task_TaskModel::getAssignTypes(),
            'value' => 0
        ));
        ...
        $collection = $this->getService('Formula')->fetchAll(
            $this->getService('Formula')->quoteInto(
                array('type = ?', ' AND cid = ?'),
                array(HM_Formula_FormulaModel::TYPE_PENALTY, $projectId)
            ),
            'name'
        );
...
/*        $this->addElement($this->getDefaultWysiwygElementName(), 'comments', array(
    ...
    public function initCuratorPoll()
    {
        ...
        $this->addElement('ajaxRadioGroup', 'questions', array(
            ...
            'dependences' => array(
                HM_Test_TestModel::QUESTIONS_BY_THEMES_SPECIFIED =>
                        $this->getView()->url(array('module' => 'meeting', 'controller' => 'list', 'action' => 'themes', 'test_id' => ''))
                        ."'+$('#module').val()
                "
            )
        ));
