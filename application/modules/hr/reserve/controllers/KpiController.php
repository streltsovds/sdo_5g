<?php
class Reserve_KpiController extends HM_Controller_Action_Reserve
{
    ...
    public function indexAction()
    {
        if (count($this->_reserve->cycle)) {
            $cycleId = $this->_reserve->cycle->current()->cycle_id;
        }
...
//                'uk.weight',
        ...
        $select
            ...
            ->where('uk.user_id = ?', $this->_reserve->user_id)
            ->group(array(
                'uk.user_kpi_id',
                'k.kpi_id',
                'k.name',
...
//                'weight' => null,
        ...
        $isKpiPassed = $this->_reserve->state_id == HM_Hr_Reserve_ReserveModel::PROCESS_STATE_OPEN;

        if ($isKpiPassed && $this->getService('Acl')->isCurrentAllowed('mca:reserve:kpi:edit')) {
...
//            'kpi_cluster_id' => $data['kpi_cluster_id'],        
        ...
        $dataKpiUser = array(
            ...
            'user_id' => $this->_reserve->user_id,
            'cycle_id' => count($this->_reserve->cycle) ? $this->_reserve->cycle->current()->cycle_id : 0,
    ...
    {
        $this->_redirector->gotoSimple('index', 'kpi', 'reserve', array('reserve_id' => $this->_reserve->reserve_id));
    }    
