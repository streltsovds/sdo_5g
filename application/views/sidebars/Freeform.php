...
class HM_View_Sidebar_Freeform extends HM_View_Sidebar_Abstract
{
    function getIcon()
    {
    ...
    function getContent()
    {
        ...
        $data['questTags'] = $this->getService('Tag')->getTags($quest->quest_id, HM_Tag_Ref_RefModel::TYPE_FORM);
        $data['questClassifiers'] = $this->getService('Classifier')->getItemClassifiers($quest->quest_id, HM_Classifier_Link_LinkModel::TYPE_FORM)->asArrayOfArrays();
        $data['quest'] = $quest;

        $data = HM_Json::encodeErrorSkip($data);

        return $this->view->partial('freeform.tpl', ['data' => $data]);
    }
}