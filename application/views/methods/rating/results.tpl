...
<?php if ($this->finalizeable) :?>            
            <a href="<?= $this->finalizeUrl ?>" target="_top" title="<?= $this->escape(_('Подтвердить окончание заполнения анкеты и выйти')) ?>" class="at-form-button at-form-finalize"><?= _('Готово') ?></a>
<?php endif;?>            
        </div>
    </div></div>
