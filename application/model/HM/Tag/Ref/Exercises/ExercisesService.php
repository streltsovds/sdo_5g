<?php
class HM_Tag_Ref_Exercises_ExercisesService extends HM_Tag_Ref_RefService
{
    ...
    {
        if ( !$this->getService('Acl')->isCurrentAllowed('mca:question:list:new') ) return null;
    ...
    {
        $rService = $this->getService('Exercises');
    ...
    public function getFilterSelect($search, Zend_Db_Select $select)
    {
        ...
        return  $select->joinInner( array('ref' => 'tag_ref'),
                            	    'ref.item_id = t.exercise_id AND ref.item_type = ' . $this->getExercisesType(),
                                    array())
    ...
    {
        return array('src' =>'/images/events/4g/64x/exercise.png','title' => _('Упражнения'));
    } 
}