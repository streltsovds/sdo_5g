<template>
  <component :is="isLegacy ? `v-app` : `div`" :class="{ isIe: isIe }" class="hm-test">
    ...
    <test-header
      ...
      v-if="testModel && !hasErrorOnLoad && testModel.title"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            testControllerProps() {
                return {
                    saveUrl: this.testModel.saveUrl,
                    finalizeUrl: this.testModel.finalizeUrl,
                    restrictUserTraversal: this.testModel.isMovementRestricted,
                    progress: this.testModel.progress,
                    time: this.testModel.time,
                    currentItem: this.testModel.currentItem,
                    results: this.testModel.results,
        ...
        methods: {
            ...
            parseLoadData(data) {
                ...
                if (data.context) {
                    ...
                    if(data.model.attempt.method === 'competence') {
                      this.isAssessment = true;
                      this.dataAssessment = data.model;
                      this.dataAssessment.user = data.info;
                    }
                }
