...
export default {
  ...
  props: ["isAssessment", "dataAssessment", "results", "finalizeInHmTestInHeader", "saveUrl", "finalizeUrl", "questions", "currentItem", "time", "progress", "restrictUserTraversal", "commentInProcessOfFilling", "limitTime", "showCommentForQuestions", "type", "modeSelfTest"], // eslint-disable-line
  data() {
    return {
      dialogMsg: null,
      isDialogShown: false,
  ...
  computed: {
    ...
    currentQuestions() {
      if(this.isAssessment) {
        if(this.dataAssessment.options.competenceUseIndicators === '1') {
          const indicators = [];
          const criteria = this.questions[this.currentIndex].criteria;
          criteria.forEach(criterion => {
            Object.values(criterion.indicators).forEach((item) => {
              indicators.push(item);
            });
          })
          return indicators
        } else {
          return this.questions[this.currentIndex].criteria;
        }
      }
      else return this.questions[this.currentIndex].questions;
    },
    testBodyProps() {
      return {
        isLast: this.isLast,
