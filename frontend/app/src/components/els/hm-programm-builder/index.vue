<template>
  <div class="hm-programm-builder">
    <v-layout wrap>
      <v-flex xs12 sm6>
        <v-card-text>
          <h2 class="title"><slot name="allItemsTitle">Все</slot></h2>
              <v-select
          ...
          <v-list v-if="subitemsFiltered.length > 0">
            <hm-transition-staggered-fade>
                <div class="padding" v-for="subitem in subitemsFiltered" :key="subitem.value">
                  <v-list-item
          ...
          <v-list>
            <v-card elevation="0" v-if="subsubjectsFiltered.length > 1">
              <v-list-group
                :value="true"
              >
                <template v-slot:activator>
                  <v-list-item-title class="list-title"><slot name="allSubjectsTitle">Учебные курсы</slot></v-list-item-title>
                </template>
                <hm-transition-staggered-fade>
                  <div class="padding" v-for="subitem in subsubjectsFiltered" :key="subitem.value">
                    <v-list-item
                      class="hm-programm-builder_item"
                      :class="[subitem.classes]"
                    >
            ...
            <v-card elevation="0" v-if="subsessionsFiltered.length > 1">
              <!-- <v-select
                      v-if="allItemsSelects.length > 1"
                      v-model="selectedItem"
                      :items="allItemsSelects"
              ...
              <v-list-group>
                <template v-slot:activator>
                  <v-list-item-title class="list-title"><slot name="allSessionsTitle">Учебные сессии</slot></v-list-item-title>
                </template>
                <hm-transition-staggered-fade>
                  <div class="padding" v-for="subitem in subsessionsFiltered" :key="subitem.value">
                    <v-list-item
                            class="hm-programm-builder_item"
                            :class="[subitem.classes]"
                    >
...
export default {
  ...
  components: { hmTransitionStaggeredFade, hmCheckbox, draggable, Icons },
  ...
  computed: {
    ...
    subitems() {
      if (
        ...
        !this.allItems[this.selectedItem].hasOwnProperty("subitems")
      )
        return null;
      return this.allItems[this.selectedItem]["subitems"];
    },
    subitemsFormatted() {
      return this.formattedItems(this.subitems);
    },
    subitemsFiltered() {
      return this.subitemsFormatted.filter(item => {
        return (
