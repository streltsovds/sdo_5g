    ...
    <div class="mod_calendar" :style="style">
      ...
      <div class="mod_calendar-days">
        <div class="mod_calendar-days__day"
          ...
          <span v-if="el > activeDayWeek" :style="getColorDate(el - activeDayWeek)">{{ el - activeDayWeek }}</span>
        </div>
      </div>
    </div>
...
<script>
import CalendarModal from "@/components/controls/hm-widget-calendar/calendar-modal/calendarModal";
export default {
  name: "calendar",
  ...
  methods: {
    getColorDate(day) {
      ...
      if(this.activeEventsDay(day)) style.background = `#${this.activeEventsDay(day)[0].color}`
      return style;
    },
    /**
