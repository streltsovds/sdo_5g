<template>
  <div id="sidebarEnduser">
    <div class="sidebar-enduser">
      <div
        v-if="!dataSidebar.subject.image"
        class="sidebar-enduser__img">
        <img :src="dataSidebar.subject.icon">
        <div class="sidebar-enduser__img-info">
          <span>{{ dataSidebar.subject.name | truncate(100)}}</span>
        </div>
      </div>
      <div
        v-else
        class="sidebar-enduser__image"
        :style="{ backgroundImage: `url(${dataSidebar.subject.image})` }"
      >
        <div class="sidebar-enduser__image-info">
          <span>{{ dataSidebar.subject.name | truncate(100)}}</span>
        </div>
      </div>
      <div
        v-if="dataSidebar.resources && dataSidebar.resources.length !== 0"
        class="sidebar-enduser__info sidebar-scroll-auto">
        <div class="sidebar-enduser__info-title">
          <svg
            width="17"
            height="20"
            viewBox="0 0 17 20"
